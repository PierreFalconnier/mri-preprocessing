"""QC metrics for preprocessed T1w images: dice overlap with the MNI brain
mask, tissue-based SNR/CNR/CJV/WM2MAX, robust (median/MAD) outlier
detection, an interactive HTML metric-distribution report, and an optional
mosaic gallery + curated (symlinked, outliers-excluded) dataset.

Moved from `quality_control/qc_advanced_v2.py` with logic unchanged; see
`mri_prep.cli` for the `mri-prep qc run` command wrapping `main()` below.
"""

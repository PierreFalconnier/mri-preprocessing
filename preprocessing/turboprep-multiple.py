...
if __name__ == "__main__":
    ...
    for input_path in tqdm(list(outputs_dict.keys())):
        ...
        os.system(
            f"mri_synthstrip -i {corrected_path} -o {skullstrip_path} --gpu > /dev/null"
        )
    ...
    if len(reg_seg_pairs) > 0:
        if os.path.exists("temp-input.txt"):
    ...
    def mask_and_normalize(paths):
        ...
        try:
            ...
            print("loading failed for", reg_path, ":", e)
            return
        ...
        if not os.path.exists(mask_path):
            try:
                ...
                print("brain extraction failed for", reg_path, ":", e)
                return
        ...
        if not os.path.exists(norm_path):
            try:
                ...
                print("normalization failed for", reg_path, ":", e)
                return

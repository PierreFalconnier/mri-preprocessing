# -*- coding: utf-8 -*-
### SCRIPT TAKEN FROM https://github.com/rmarkello/pypmi/blob/master/pypmi/data/heuristic.py
### IT IS 7 YEARS OLD AND MAY NOT BE UP TO DATE WITH THE CURRENT VERSION OF HEUDICONV. USE WITH CAUTION!
"""
...
T1W_SERIES = [
    ...
    "SAG T1 3D FSPGR",
    "SAG FSPGR 3D SAG 3D FSPGR BRAVO STRAIGHT",
...
def infotodict(seqinfo):
    ...
    t1w_grappa = create_key(
        "{bids_subject_session_dir}/anat/{bids_subject_session_prefix}_acq-grappa2_run-{item:02d}_T1w"
    )  # noqa
    t1w_adni = create_key(
        "{bids_subject_session_dir}/anat/{bids_subject_session_prefix}_acq-adni_run-{item:02d}_T1w"
    )  # noqa
    t2w = create_key(
    ...
    for s in seqinfo:
        ...
        # if we don't match _anything_ then we want to know!
            ...
            lgr.warning(
                "Skipping unrecognized series description: {}".format(
...
def custom_callable(*args):
    ...
    from heudiconv.utils import TempDirs, load_json, set_readonly, treat_infofile
    ...
    for echo, (nifti, json) in zip(echonums, bids_pairs):
        ...
        outname, scaninfo = (new_prefix + "." + outtype, new_prefix + scaninfo_suffix)
...
def safe_movefile(src, dest, overwrite=False):
    ...
    try:
        ...
        lgr.warning(
            "Tried moving %s to %s but %s " % (src, dest, dest)
